...
- 浏览器访问服务器要跨域，服务器访问服务器不用

- *浏览器有同源策略**，所以页面**在* `http://localhost:9000` *时，JS* *直接请求* `http://localhost:3000` *可能被**拦**截。*
  *开发时我****本地起一个代理服务**（例如* `webpack-dev-server`*），**浏览器只请求* `http://localhost:9000/api/...`*，**这样**对浏览器来说是**同源请求**，不会触发跨域拦截**。*
  *代理服务**收到请求后，再在**服务端去请求真实**后端* `http://localhost:3000/...`*（服务器**到服务器请求不受**同源**策略限制），拿到**响应后再转发回浏览器。*
  *所以本质**是**“浏览器 -> 同源代理 -> 后**端 -> 代理 ->* *浏览器**”的**转发**链路。*

- ##### 这个代理一定“在本地开服务器”吗？

  - **开发阶段**：通常是“本地开服务器”（webpack-dev-server / 本地 Node 代理），你说的没问题
  - **生产阶段**：常见是“线上用 Nginx/网关开服务器”，浏览器访问的是线上域名
